...
def make_app(from_account='acc-002', to_account='acc-003', amount=500000):
    ...
    builder.add_node('transfer_agent', build_transfer_graph(fake_extract))
...
r = app.invoke(Command(resume='approve'), config=cfg)
...
r = app.invoke(Command(resume='reject'), config=cfg)
